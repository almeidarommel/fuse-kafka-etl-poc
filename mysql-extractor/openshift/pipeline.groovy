        ...
        stage("Initilize") {
            ...
            steps {
                script {
                    ...
                    pom = readMavenPom(file: './mysql-extractor/pom.xml')
                    ...
                    openshift.withCluster() {
                        openshift.withProject("${params.PROJECT_NAME}") {
                            ...
                            if (!openshift.selector("bc", "${artifactId}").exists()) {
                                openshift.newBuild("--name=${artifactId}", "--docker-image=registry.access.redhat.com/redhat-openjdk-18/openjdk18-openshift:${version}", "--binary")                            
                            }
                        }
                    }
                }
            ...
            steps {
                script {
                    openshift.withCluster() {
                        openshift.withProject("${params.PROJECT_NAME}") {
                            echo "Using project: ${openshift.project()}"
                            openshift.selector("bc", "${artifactId}").startBuild("--from-file=mysql-extractor/target/${artifactId}-${version}.jar", "--wait=true")
                        }
                    }
                }
            }
                    ...
                    openshift.withCluster() {
                        openshift.withProject("${params.PROJECT_NAME}") {
                              if (!openshift.selector('dc', "${artifactId}").exists()) {
                                openshift.newApp("--template=fuse-mysql-extractor", 
                                " -p APP_TAG=${version}",
                                ...
                                sh  '''
                                    ...
                                    oc rollout latest dc/mysql-extractor
